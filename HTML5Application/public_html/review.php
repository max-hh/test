...
$request = $mysqli->query("SELECT COUNT(*) AS anzahl FROM review WHERE transaktion=$id");
$var = mysqli_fetch_object($request);
if ($var->anzahl == 1){

echo '</br>Du hast die Transaktion bereits bewertet';
}else{
$select = $mysqli->query("SELECT userkennungT, id FROM finale WHERE id=$id");
echo "<p>Bitte bewerte die Tansaktion und den User</p>";
while($row = mysqli_fetch_object($select))
         {

         $userkennungT ="$row->userkennungT";
$res = $mysqli->query("SELECT firstname FROM carrier WHERE user='".$userkennungT."'");
...
echo "<form name='Formular' action='summary.php' method='post'>";
 echo "<p><input type='hidden' name='user' value='$userkennungT'></p>";
...
echo "<p><input type='text'name='title' value='' Placeholder='Revie-Title'></p>";
echo "<p><input type='text' name='reviewtext' value='' Placeholder='Bewertungs-Text' style='height:80px;'></p>";
