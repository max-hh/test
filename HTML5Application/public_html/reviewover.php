...
$userT = $_SERVER['QUERY_STRING'];
$userkennungT = decrypt($userT, $key);
...
$res = $mysqli->query("SELECT sum(value) FROM review WHERE user='".$userkennungT."'");
$s=current(mysqli_fetch_assoc($res));


$req= $mysqli->query("SELECT * FROM review WHERE user='".$userkennungT."'");
