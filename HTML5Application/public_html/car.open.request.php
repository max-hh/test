...
$request = $mysqli->query("SELECT aid, userkennungA, gegenstand, price, hight, width, depth, info, city, str, plz, land, date, arrcity, arrstr, arrplz, arrland, arrdate, userkennungT, fahrzeug, licence, start, startdate, ziel, tarrdate FROM temp WHERE userkennungT='".$user."'");
...
while($rows = mysqli_fetch_object($res))
  ...
  echo "<tr><td>FAHRZEUG:</td>";
   ...
   echo "<form name='Formular' action='car.summary.php' method='post'>";
    ...
    echo "<tr><td><p><a href='car.message.php'>Kontakt</a></td><td><a href='car.decline.php?aid=$aid'>Ablehnen</a></td><td></td><td align='right'><input type='submit' name='submit' value='Akzeptieren'></p></td></tr>";
    echo "</table>";
                        }
                           }
